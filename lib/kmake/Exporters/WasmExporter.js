...
class WasmExporter extends Exporter_1.Exporter {
    constructor(options) {
        ...
        this.make = new MakeExporter_1.MakeExporter(options, compiler, compiler, compiler, compilerFlags, compilerFlags, '--target=wasm32 -nostdlib -matomics -mbulk-memory "-Wl,--import-memory,--shared-memory"', '.wasm');
        this.ninja = new NinjaExporter_1.NinjaExporter(options, compiler, compiler, compiler, compilerFlags, compilerFlags, '--target=wasm32 -nostdlib -matomics -mbulk-memory "-Wl,--import-memory,--shared-memory"', '.wasm');
    }
