...
const cpuCores = require('os').properCpuCount();
let _global = global;
...
async function exportKoremakeProject(from, to, platform, korefile, retro, options) {
    ...
    let project;
    try {
        project = await Project_1.Project.create(from, to, platform, korefile, retro);
...
async function run(options, loglog) {
    ...
    Project_1.Project.koreDir = options.kinc;
        ...
        if ((options.customTarget && options.customTarget.baseTarget === Platform_1.Platform.Linux) || options.target === Platform_1.Platform.Linux || options.target === Platform_1.Platform.FreeBSD) {
            ...
            switch (options.visualstudio) {
                ...
                case VisualStudioVersion_1.VisualStudioVersion.VS2015:
                    ...
                    break;
                default:
                    const vspath = child_process.execFileSync(path.join(process.env['ProgramFiles(x86)'], 'Microsoft Visual Studio', 'Installer', 'vswhere.exe'), ['-latest', '-property', 'installationPath'], { encoding: 'utf8' });
