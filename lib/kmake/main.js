...
async function compileShader(projectDir, type, from, to, temp, platform, builddir, shaderversion) {
    return new Promise((resolve, reject) => {
        ...
        if (Project_1.Project.kincDir && !fs.existsSync(libsdir)) {
...
async function exportKoremakeProject(from, to, platform, korefile, retro, veryretro, options) {
    ...
    if (options.vscode) {
        ...
        exporter = new EmscriptenExporter_1.EmscriptenExporter(options);
...
async function run(options, loglog) {
    ...
    if (options.compile && solutionName !== '') {
        ...
        let make = null;
        if (isPlatform(options, Platform_1.Platform.Linux) || isPlatform(options, Platform_1.Platform.Wasm)) {
            make = child_process.spawn('ninja', [], { cwd: path.join(options.to, options.buildPath) });
        }
        else if (isPlatform(options, Platform_1.Platform.FreeBSD) || isPlatform(options, Platform_1.Platform.Pi) || isPlatform(options, Platform_1.Platform.Emscripten)) {
        ...
        if (make !== null) {
            try {
                ...
                if (typeof (err) === 'number') {
                    ...
                    if (isPlatform(options, Platform_1.Platform.Linux) || isPlatform(options, Platform_1.Platform.Wasm) || isPlatform(options, Platform_1.Platform.Pi)) {
